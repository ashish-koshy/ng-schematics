...
import { Schema as AppShellOptions } from '@schematics/angular/app-shell/schema';
...
function getSourceFile(host: Tree, path: string): ts.SourceFile {
    const content = readText(host, path);
...
function getServerModulePath(
    ...
    const mainSource = getSourceFile(
        host,
        join(normalize(sourceRoot), mainPath)
    );
...
function getComponentTemplate(
    ...
    if (tmplInfo.templateProp) {
        ...
        const templateUrl = (
            tmplInfo.templateUrlProp.initializer as ts.StringLiteral
        ).text;
        const dir = dirname(normalize(compPath));
        const templatePath = join(dir, templateUrl);
        try {
            template = readText(host, templatePath);
        } catch {}
    }
...
function getBootstrapComponentPath(host: Tree, mainPath: string): string {
    ...
    const arrLiteral =
        bootstrapProperty.initializer as ts.ArrayLiteralExpression;
...
function addAppShellConfigToWorkspace(options: AppShellOptions): Rule {
    return (host, context) => {
        if (host) console.log('Adding app shell config to workspace');
        if (!options.route) {
        ...
        return updateWorkspace((workspace) => {
            ...
            const serverConfigKeys =
                project.targets.get('server')?.configurations ?? {};
            const buildConfigKeys =
                project.targets.get('build')?.configurations ?? {};
            ...
            project.targets.add({
                ...
                defaultConfiguration: configurations['production']
                    ? 'production'
                    : undefined,
                options: {
                    route: options.route,
...
function addServerRoutes(options: AppShellOptions): Rule {
    return async (host: Tree) => {
        ...
        if (!clientServerTarget) {
            throw new Error(
                'Universal schematic did not add server target to client project.'
            );
        }
        const clientServerOptions =
            clientServerTarget.options as unknown as ServerBuilderOptions;
        if (!clientServerOptions) {
            throw new SchematicsException(
                'Server target does not contain options.'
            );
        }
...
export default function (options: AppShellOptions): Rule {
    return async (tree) => {
        ...
        if (
            !clientProject ||
            clientProject.extensions.projectType !== 'application'
        ) {
            throw new SchematicsException(
