...
describe('App Shell Schematic', () => {
    ...
    it('should add app shell configuration', async () => {
        ...
        expect(target.configurations.development.browserTarget).toEqual(
            'bar:build:development'
        );
        expect(target.configurations.development.serverTarget).toEqual(
            'bar:server:development'
        );
        expect(target.configurations.production.browserTarget).toEqual(
            'bar:build:production'
        );
        expect(target.configurations.production.serverTarget).toEqual(
            'bar:server:production'
        );
    });
    ...
    it('should not fail when AppModule have imported RouterModule already', async () => {
        const updateRecorder = appTree.beginUpdate(
    ...
    it('should work after adding nguniversal', async () => {
        ...
        tree = await schematicRunner
            .runSchematicAsync('app-shell', defaultOptions, tree)
            .toPromise();
        const filePath = '/projects/bar/src/app/app.server.module.ts';
        const content = tree.readContent(filePath);
        expect(content).toMatch(
    ...
    it('should create the shell component', async () => {
        ...
        expect(
            tree.exists(
                '/projects/bar/src/app/app-shell/app-shell.component.ts'
            )
        ).toBe(true);
        const content = tree.readContent(
